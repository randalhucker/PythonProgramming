...
# RQ2
    def go_to(self, direction):
        ...
        if direction == self.place.name:
            print(f"Can't go to {direction} from {direction}.")
        ...
        if direction in self.place.exits:
            match direction:
                case 'Bearcat Cafe':
                    self.place = bcc
                case 'Tangeman University Center':
                    self.place = tuc
                case 'CCM':
                    self.place = ccm
                case 'ERC':
                ...
                case 'Baldwin':
                    self.place = baldwin
                case 'Gamestop':
                    self.place = game_store
                case 'Starbucks':
                    self.place = starbucks
                case 'Swift Hall':
                    self.place = swift
                case 'Rhodes Hall':
                    self.place = rhodes
...
albert = Character(
    'Albert', 'Randy went to DAAP for potluck. You can find her there.')
...
# Things:
wallet = Thing(
    'Wallet', "Looks like Randy's wallet. We should return it to her.")
